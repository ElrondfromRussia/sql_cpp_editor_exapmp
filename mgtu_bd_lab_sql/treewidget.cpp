...
{
    // это конструктор, вроде, по названиям методов все понятно
...
QSqlDatabase TreeWidget::currentDatabase() const
{
...
void TreeWidget::setActive(QTreeWidgetItem *item)
{
...
void TreeWidget::on_tree_itemActivated(QTreeWidgetItem *item, int column)
{
    if (!item)
        return;
...
void TreeWidget::on_tree_currentItemChanged(QTreeWidgetItem *current, QTreeWidgetItem *previous)
{
